...
import 'package:efood_kitchen/features/home/widgets/slider_button_widget.dart';
...
class StatusChangeCustomButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Get.find<OrderController>().orderStatus == 'confirmed' || orderStatus == 'confirmed'? Container(
    ...
    ) : Get.find<OrderController>().orderStatus == 'cooking'  || orderStatus == 'cooking'? Container(
      height: ResponsiveHelper.isSmallTab() ? 40 : 50,
      margin: const EdgeInsets.all(Dimensions.paddingSizeSmall),
      padding: const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeExtraSmall),
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall),
        border: Border.all(color: Theme.of(context).secondaryHeaderColor.withValues(alpha:.5)),
        color: Theme.of(context).cardColor,),
      child: Transform.rotate(
        angle: Get.find<LocalizationController>().isLtr ? pi * 2 : pi, // in radians
        child: Directionality(
